...
static void fill_dhcpc_pipe_data (struct dhcp_packet *packet, unsigned char *output_buffer, int *output_len) {
	int g;
	uint8_t *option;
	unsigned char opt_code, byte;
	uint8_t type;
	int optlen, len;
	char *dest;
	
	uint8_t critical_opts[] = {
		0x01, /* DHCP_SUBNET */
		0x03, /* DHCP_ROUTER */
		0x06, /* DHCP_DNS_SERVER */
		0x0c, /* DHCP_HOST_NAME */
		0x0f, /* DHCP_DOMAIN_NAME */
		0x2a, /* DHCP_NTP_SERVER */
		0
	};
	
	uint8_t opts_types[] = {
		OPTION_IP,
		OPTION_IP | OPTION_LIST,
		OPTION_IP | OPTION_LIST,
		OPTION_STRING_HOST,
		OPTION_STRING_HOST,
		OPTION_IP | OPTION_LIST,
	...
	if (packet == NULL) {
		/* Enviar un cero para indicar que no hay ni IP o server address */
		byte = 0;
	...
	byte = DHCPC_PIPEOUT_HAS_IP | DHCPC_PIPEOUT_HAS_OPTS;
	
	if (packet->siaddr_nip) {
		byte |= DHCPC_PIPEOUT_HAS_SERVER_IP;
	}
	
	/* Enviar la lista de banderas activadas para esta configuración */
	output_buffer[*output_len] = byte;
	(*output_len)++;
	
	/* Enviar la IP */
	memcpy (&output_buffer[*output_len], &packet->yiaddr, 4);
	*output_len += 4;
	
	/* Enviar el servidor IP si existe */
	if (packet->siaddr_nip) {
		memcpy (&output_buffer[*output_len], &packet->siaddr_nip, 4);
		*output_len += 4;
	}
	
	g = 0;
	while (critical_opts[g] != 0) {
		opt_code = critical_opts[g];
		option = udhcp_get_option(packet, opt_code);
		
		if (option) {
			type = opts_types[g] & OPTION_TYPE_MASK;
			...
			len = option[-1];
			if (opts_types[g] & OPTION_LIST) {
				/* Para las opciones que son lista, enviar un byte indicando cuantos elementos hay en la lista */
				byte = len / optlen;
				output_buffer[*output_len] = byte;
				(*output_len)++;
			}
			...
			while (len >= optlen) {
				switch (type) {
				case OPTION_IP:
					memcpy (&output_buffer[*output_len], option, 4);
					*output_len += 4;
					break;
				case OPTION_STRING_HOST:
					...
					if (type == OPTION_STRING_HOST && !good_hostname(dest)) {
						memcpy (&output_buffer[*output_len], "bad", 4);
						*output_len += 4;
					} else {
						memcpy (&output_buffer[*output_len], dest, len + 1);
						*output_len += (len + 1);
					}
					break;
				}
				
				if (type == OPTION_STRING_HOST) {
					break;
				}
	...
	/* Enviar un 255 para indicar el final de las opciones */
	byte = 255;
	output_buffer[*output_len] = byte;
	(*output_len)++;
}
...
static void udhcp_run_script (int output_pipe, struct dhcp_packet *packet, const char *name) {
	unsigned char type;
	...
	if (strcmp (name, "deconfig") == 0) {
		...
		type = 3;
	} else if (strcmp (name, "renew") == 0) {
		type = 4;
	} else if (strcmp (name, "nak") == 0) {
		type = 5;
	}
	
	output_buffer[output_len] = type;
	output_len++;
	
	fill_dhcpc_pipe_data (packet, output_buffer, &output_len);
	
	/* Enviar todos los datos de un solo jalón, para que el otro extremo los lea en un solo bloque */
	write (output_pipe, output_buffer, output_len);
}
...
static NOINLINE int send_discover(uint32_t xid, uint32_t requested)
{
	...
	fprintf (stderr, "Sending discover...");
	return raw_bcast_from_client_config_ifindex(&packet);
}
 ...
 */
	...
	fprintf (stderr, "Sending select for %s...", inet_ntoa(addr));
	return raw_bcast_from_client_config_ifindex(&packet);
}
...
static NOINLINE int send_decline(/*uint32_t xid,*/ uint32_t server, uint32_t requested)
{
	...
	fprintf(stderr, "Sending decline...");
	return raw_bcast_from_client_config_ifindex(&packet);
}
#endif
...
static int send_release(uint32_t server, uint32_t ciaddr)
{
	...
	fprintf (stderr, "Sending release...");
...
static void perform_renew(int pipe_fd)
{
	fprintf (stderr, "Performing a DHCP renew");
	switch (state) {
	case BOUND:
	...
	case RENEW_REQUESTED: /* impatient are we? fine, square 1 */
		/* FIXME: Correr script aquí */
		udhcp_run_script(pipe_fd, NULL, "deconfig");
	case REQUESTING:
	case RELEASED:
...
static void perform_release(int pipe_fd, uint32_t server_addr, uint32_t requested_ip)
{
	...
	if (state == BOUND || state == RENEWING || state == REBINDING) {
		...
		fprintf (stderr, "Unicasting a release of %s to %s",
				inet_ntoa(temp_addr), buffer);
		send_release(server_addr, requested_ip); /* unicast */
		/* FIXME: Aquí SCRIPT */
		udhcp_run_script (pipe_fd, NULL, "deconfig");
	}
	fprintf (stderr, "Entering released state");
...
int udhcpc_main(int pipe_fd, char *interface)
{
	uint8_t *temp, *message;
	// str_V se usa como argumento -V,--vendorclass VENDOR
	// 
	...
	n = 0x06; // DHCP_DNS_SERVER
	client_config.opt_mask[n >> 3] |= 1 << (n & 7);
	
	n = 0x0c; // DHCP_HOST_NAME
	client_config.opt_mask[n >> 3] |= 1 << (n & 7);
	
	n = 0x0f; // DHCP_DOMAIN_NAME
	client_config.opt_mask[n >> 3] |= 1 << (n & 7);
	
	n = 0x2a; // DHCP_NTP_SERVER
	client_config.opt_mask[n >> 3] |= 1 << (n & 7);
#if 0
	while (list_x) {
 ...
 leasefail:
				/* FIXME: AQUI SCRIPT */
				udhcp_run_script (pipe_fd, NULL, "leasefail");
			...
			case REBINDING:
				...
				fprintf (stderr, "Lease lost, entering init state");
				/* FIXME: AQUI SCRIPT */
				udhcp_run_script (pipe_fd, NULL, "deconfig");
				state = INIT_SELECTING;
				client_config.first_secs = 0; /* make secs field count from 0 */
		...
		case SIGUSR1:
			...
			continue;
		case SIGUSR2:
			perform_release (pipe_fd, server_addr, requested_ip);
			timeout = INT_MAX;
			continue;
		case SIGTERM:
...

		{
			...
			if (len == -1) {
				...
				fprintf (stderr, "Read error: %s, reopening socket", strerror(errno));
...
//FIXME: need to also check that last 10 bytes are zero
		...
		if (message == NULL) {
			fprintf (stderr, "no message type option, ignoring packet");
			continue;
		}
 ...
 */
				...
				if (!temp) {
					fprintf (stderr, "no server ID, using 0.0.0.0");
				} else {
		...
		case REBINDING:
			if (*message == DHCPACK) {
				...
				if (!temp) {
					fprintf (stderr, "no lease time with ACK, using 1 hour lease");
					lease_seconds = 60 * 60;
				} else {
					/* it IS unaligned sometimes, don't "optimize" */
				...
				fprintf (stderr, "Lease of %s obtained, lease time %u",
					inet_ntoa(temp_addr), (unsigned)lease_seconds);
				requested_ip = packet.yiaddr;
				/* FIXME: Aquí SCRIPT */
				udhcp_run_script (pipe_fd, &packet, state == REQUESTING ? "bound" : "renew");
			...
			if (*message == DHCPNAK) {
				...
				fprintf (stderr, "Received DHCP NAK");
				/* FIXME: AQUI SCRIPT */
				udhcp_run_script (pipe_fd, &packet, "nak");
				if (state != REQUESTING) {
					/* FIXME: AQUI SCRIPT */
					udhcp_run_script (pipe_fd, NULL, "deconfig");
				}
